...
export default function GamePage() {
  ...
  const handleStart = async () => {
    ...
    // Play main background music - only once
    if (!audioRef.current) {
      ...
      <div className="min-h-screen bg-linear-to-b from-slate-900 to-slate-800 text-white flex items-center justify-center">
        <div className="text-center space-y-8">
          ...
          <p className="text-slate-300 text-lg">
            Game cờ tỷ phú phiên bản triết học
          </p>
          <button
            type="button"
            onClick={handleStart}
          ...
          <p className="text-slate-400 text-sm">
            Nhấn để bật nhạc nền và vào game
          </p>
        </div>
      </div>
    );
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="space-y-2">
            ...
            <h1 className="text-3xl font-bold leading-tight">
              Marx-opoly + cấu hình nhóm
            </h1>
            <p className="text-sm text-slate-200/90">
              Đăng ký nhóm, chọn màu và avatar ngay tại tab Game; cấu hình này
              sẽ được dùng chung với thanh top bar và các mode khác.
            </p>
